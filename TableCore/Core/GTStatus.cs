...
namespace TableCore
{
    public class GTStatus
    {
        ...
        public int DataStartRow { get { return (m_SkipTypeDefine ? 1 : 3) + m_StartRow; } }
        public bool UnlimitMode { get; set; }
        /*
         * {
         * "class1":{"p1":"type","P2":"type"}
         * "class2":{"p1":"type",...}
         * }
         */
        JsonData mModifyPropertyType; // <propertyName, propertyType>
        GTConfig mCfg;
        ...
        public void ModifyClass(string modifyFile)
        {
            ...
            if (string.IsNullOrEmpty(file))
                return;
            mModifyPropertyType = JsonMapper.ToObject(file);// JsonConvert.DeserializeObject<JObject>(file);
        }
        ...
        public bool GetModify(string classname, string property, out string type)
        {
            ...
            JsonData tab = null;
            tab = mModifyPropertyType[classname];
            type = tab == null ? null : (string)tab[property];
        ...
        public void UseTable(string table, string className, int startRow, int startCol, bool alwaysuse = false, bool skipTypeDef = false)
        {
            int tab = mTables.IndexOf(table);
            UseTable(tab, className, startRow, startCol, alwaysuse, skipTypeDef);
        }
        ...
        public void UseTable(int tabIndex, string className, int startRow, int startCol, bool alwaysuse = false, bool skipTypeDef = false)
        {
            ...
            m_StartRow = startRow;
            m_StartCol = startCol;
            if (skipTypeDef && !UnlimitMode && (mModifyPropertyType == null || mModifyPropertyType[className] == null))
                throw new Exception(string.Format("没有找到对象（{0}）的定义文件", className));
            m_SkipTypeDefine = skipTypeDef;
        ...
        public void UpdateData(JsonData data)
        {
            ...
            foreach(var pro in data.Keys)
            {
                int col = FindCol(pro);
                if(col != -1)
                {
                    object s = rowdata[col];
                    object v = data[pro];// pro.Value.ToString();
        ...
        void GenerateData(Action<int, string> progressHandler, LinkedList<GenData> datas, out int dataNums, LinkedList<IExportData> exports, out int exportNums)
        {
            ...
            var row0 = DataStartRow;
            int rows = m_UseTab.Rows.Count - row0;
            ...
            for (int i = 0; i < rows; i++)
            {
                ...
                if (ClassMod.AutoId)
                    entity.id = r;
                bool skip = false;
                for (int j = p0; j < ClassMod.PropertyCount; j++)
                {
                    var property = ClassMod.GetProperty(j);
                    if (property.Ignore)
                        continue;
                    var array = row.ItemArray;
                    string dt = Utils.GetString(array != null && j + col0 < array.Length ? array[j + col0] : null);// Utils.GetString(row[j + m_StartCol]);
